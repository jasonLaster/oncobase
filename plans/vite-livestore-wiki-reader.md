...
| Command palette and page finder | `web/src/components/command-palette.tsx` | `apps/wiki-vite/src/shell/CommandPalette.tsx` | Command-palette modal/search/tabs/list/footer shell and button/link result rows now live in `packages/wiki-shell`; Vite still owns LiveStore-backed pages/outline/assets/tag/recent/action/debug data and side effects. Remaining work is virtualization parity for large page lists and final mobile/focus-state polish against the current app. | P1 partially done |
...
| --- | --- | --- |
| Auth/session UX | Hash-preserving session recovery, scope switching, login return-target copy, and a more polished accessible login panel have landed. Remaining: expired-session edge cases beyond current public fallback and production auth copy review. | Current focused tests cover hash-preserving redirect, public fallback, session store separation, cache-key rotation, and login error semantics. |
| Downloads and file actions | Vite now owns scoped `/api/page-copy`, `/api/file`, `/api/download?type=full` markdown-bundle behavior for the standalone path, and page chrome exposes both current-page and full-wiki download actions. Remaining: production full-export format decisions and richer asset-specific overflow actions. | Backend tests cover scoped markdown downloads; page chrome tests fetch page markdown through the Vite API boundary and assert the full-wiki action URL. |
| Search/AI result UX | Result cards keep native links, support arrow/Enter keyboard selection, show tag context, preserve AI summaries/relevance, publish search metrics to the observability buffer, and now render AI source chips plus sensitive-session badges. Remaining: richer snippet highlighting and live session-auth leak checks in deployed preview. | Search route tests cover keyboard selection, AI tags/summaries, source chips, sensitive badges, error fallback, and session-only local results. |
| Command palette and sidebar accessibility | Palette mode buttons expose pressed state, keyboard highlighting works without losing native button/link semantics, command search uses combobox/listbox wiring, active rows scroll into view, sidebar directories expose expanded state, current page links expose `aria-current`, and the mobile sheet is a dialog. Remaining: very large tree performance, virtualization, and a stricter focus-trap audit. | Focused tests cover palette keyboard selection, tab state, active semantics, sidebar expansion/current state, outline jumps, and asset actions. |
