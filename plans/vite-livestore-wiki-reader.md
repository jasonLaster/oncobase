...
| Sidebar tree and mobile nav | `web/src/components/sidebar.tsx`, `bottom-nav.tsx` | `apps/wiki-vite/src/shell/Navigation.tsx` | `WikiSidebar`, `WikiTree`, `WikiMobileNavigation`, and tree helpers now live in `packages/wiki-shell`, with Vite feeding LiveStore tree data, active route, file URLs, and persisted expansion state. Remaining work is visual tuning against the current app's exact spacing/iconography and any chat-specific mobile nav integration. | P1 partially done |
...
| Search and chat chrome | Current search/chat route components and shared composer pieces | `apps/wiki-vite/src/pages/SearchPage.tsx`, Vite chat route | Search page chrome now uses `packages/wiki-shell` primitives for the route frame, form, mode toggle, result status, empty/error state, and result rows while Vite keeps backend calls and React Router adapters. Chat route shell now uses `packages/wiki-shell` primitives for the page, sidebar, conversation list rows, main lane, and loading/archived states while Vite keeps Convex/runtime wiring. Remaining work is deeper chat row/action parity, composer polish, source-link cards, and shared action menus. | P1 partially done |
