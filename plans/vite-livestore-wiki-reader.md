...
4. `packages/wiki-shell` owns the first reusable Diana reader shell slice: right rail/outline, layout rail collapse/resize state, shared outline helpers, header chrome primitives, page chrome primitives, shell CSS, and the smart-table rail event contract. It should grow next to include sidebar/mobile navigation visuals, shared theme/prose CSS, loading states, and visual interaction primitives.
