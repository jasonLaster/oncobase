...
| Command palette and page finder | `web/src/components/command-palette.tsx` | `apps/wiki-vite/src/shell/CommandPalette.tsx` | Command-palette modal/search/tabs/list/footer shell now lives in `packages/wiki-shell`; Vite still owns LiveStore-backed pages/outline/assets/tag/recent/action/debug result rendering. Remaining work is shared row primitives, virtualization parity for large page lists, and final focus-state polish against the current app. | P1 partially done |
...
| Search and chat chrome | Current search/chat route components and shared composer pieces | `apps/wiki-vite/src/pages/SearchPage.tsx`, Vite chat route | Search page chrome now uses `packages/wiki-shell` primitives for the route frame, form, mode toggle, result status, empty/error state, and result rows while Vite keeps backend calls and React Router adapters. Remaining work is chat route sidebar/mobile layout, composer polish, source-link cards, and shared action menus. | P1 partially done |
