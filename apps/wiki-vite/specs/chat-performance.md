...
In scope:
- The chat route at `apps/wiki-vite/src/app/(main)/chat/`, including `[id]/page.tsx`, `[id]/client.tsx`, and `_components/chat-interface.tsx`.
- The chat API route at `apps/wiki-vite/src/app/api/chat/route.ts`.
...
Frontend findings (paths under `apps/wiki-vite/src/app/(main)/chat/`):
...
Backend findings (`apps/wiki-vite/src/app/api/chat/route.ts`):
...
```ts
// apps/wiki-vite/src/lib/chat/types.ts (new)
...
`apps/wiki-vite/src/app/api/chat/route.ts` is reorganized to look like this (sketch — full code in the plan):
