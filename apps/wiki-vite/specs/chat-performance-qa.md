...
# 2. Browser perf scenarios (writes apps/wiki-vite/e2e/.perf/P0-*.json)
bun playwright test e2e/chat-perf.spec.ts --project=tests
```

CI: `e2e/chat-perf.spec.ts` runs with `apps/wiki-vite/e2e/.perf/baseline.json` as the gate. Failures land in `apps/wiki-vite/e2e/__diff_output__/`.
...
When the suite runs, paste the JSON dumps from `apps/wiki-vite/e2e/.perf/` here and update the rows below.
...
Once captured, paste the JSON dumps from `apps/wiki-vite/e2e/.perf/` here and update the Phase 0 row in the table.
...
Code: `apps/wiki-vite/src/app/api/chat/_flusher.ts` (new), `apps/wiki-vite/src/app/api/chat/route.ts` (smoothStream + flusher delegation).
...
Code: `apps/wiki-vite/src/components/chat/streaming-markdown.tsx`, behind `NEXT_PUBLIC_CHAT_STREAMDOWN=1`.
...
Code: `apps/wiki-vite/src/app/api/chat/_system-prompt-cache.ts` (60s TTL, single in-flight promise), `messages.messageId` + `by_message_id` index, idempotent `saveMessages`, `createIdGenerator`, `x-request-id` header + log prefix.
