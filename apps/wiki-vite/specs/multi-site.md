...
`apps/wiki-vite/src/proxy.ts` resolves the active site from the `Host` header
...
2. `apps/wiki-vite/.local-hosts.json` fixture
...
`apps/wiki-vite/convex/lib/site.ts` exports `requireSite(ctx, siteSlug)`:
...
request via `siteDataFromRequest(request)` in `apps/wiki-vite/src/lib/site-data.ts`.
...
`apps/wiki-vite/src/lib/blob.ts`'s `sitePut` helper. ESLint bans direct
...
vault. The CLI hits `apps/wiki-vite/src/app/api/publish/[...step]/route.ts`
...
`apps/wiki-vite/e2e/multi-site-isolation.spec.ts` exercises six invariants
