...
Artifacts shipped:
- `apps/wiki-vite/src/lib/chat/perf.ts` — small typed sink, no runtime cost in prod beyond a sampled `performance.mark`.
- `apps/wiki-vite/e2e/chat-perf.spec.ts` — Playwright scenarios.
- `apps/wiki-vite/scripts/chat-bench.ts` — CLI bench.
...
- Extract `createConvexFlusher(conversationId, { intervalMs: 250 })` to `apps/wiki-vite/src/app/api/chat/_flusher.ts`. It owns:
...
Artifacts shipped:
- `apps/wiki-vite/src/app/api/chat/_flusher.ts` (~120 LOC).
- `apps/wiki-vite/src/app/api/chat/route.ts` slimmed by the inline flush logic.
...
- Updated `apps/wiki-vite/src/app/api/chat/_flusher.ts` and `convex/conversations.ts`.
...
Work:
- Update `apps/wiki-vite/src/app/api/chat/route.ts` to return `result.toUIMessageStreamResponse({ generateId })`. The existing Convex flusher still runs as a side effect.
...
Artifacts shipped:
- `apps/wiki-vite/src/app/(main)/chat/_components/chat-client.tsx` (new, holds `useChat`).
...
Artifacts shipped:
- `apps/wiki-vite/src/app/(main)/chat/_components/messages.tsx` — `<PriorMessages>` + `<StreamingMessage>`.
- `apps/wiki-vite/src/app/(main)/chat/_components/message-row.tsx` — memoized row.
- `apps/wiki-vite/src/app/(main)/chat/_components/parts/*.tsx` — one file per part renderer.
...
Artifacts shipped:
- `apps/wiki-vite/src/components/chat/markdown.tsx` — thin wrapper over `<Streamdown>`.
- Two ported remark plugins under `apps/wiki-vite/src/lib/markdown/`.
...
Work:
- `npx ai-elements@latest add conversation prompt-input`. Components land under `apps/wiki-vite/src/components/ai-elements/`.
...
Artifacts shipped:
- `apps/wiki-vite/src/app/(main)/chat/_components/composer.tsx` (new).
...
Artifacts shipped:
- `apps/wiki-vite/src/lib/chat/system-prompt-cache.ts`.
