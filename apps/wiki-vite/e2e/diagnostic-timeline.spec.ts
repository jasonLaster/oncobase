...
test.describe("diagnostic timeline", () => {
  ...
  test("renders sleeves, week ticks, hover tooltips, diagnostics links, and zoom state", async ({
    ...
    await expect(page.getByRole("button", { name: "Show full timeline" })).toHaveCount(0);
    await expect(
    ...
    const rangeAfterHorizontalScroll = await timeline.getAttribute("data-visible-range");
    ...
    const signateraTooltip = page.getByTestId("timeline-tooltip-signatera-2026-05-28");
    ...
    const petTooltip = page.getByTestId("timeline-tooltip-cu-grip-petct-2026-06-10");
