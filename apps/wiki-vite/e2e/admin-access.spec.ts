...
test.describe("admin access management", () => {
  ...
  test("autosaves role changes from the users table", async ({
    ...
    await expect(page.getByRole("link", { name: /Roles Create and edit/ })).toHaveAttribute("href", "/admin/roles");
    ...
    await expect(rolesSection.getByRole("row", { name: new RegExp(roleName) })).toBeVisible();

    await page.setViewportSize({ width: 500, height: 800 });
    const roleTableWidths = await page.locator("#roles table").evaluate((element) => ({
      clientWidth: element.parentElement?.clientWidth ?? 0,
      scrollWidth: element.parentElement?.scrollWidth ?? 0,
    }));
    expect(roleTableWidths.scrollWidth).toBeGreaterThan(roleTableWidths.clientWidth);
    await page.setViewportSize({ width: 1280, height: 720 });

    let roleId = "";
    await expect
      .poll(async () => {
        const roles = await convex.query(api.access.listRoles, { siteSlug: SITE_SLUG });
    ...
    await expect(pagesSection.getByText(`Visible to ${roleName}`).first()).toBeVisible();
    ...
    await page.getByLabel("Search pages").fill("included");
