...
Convex is the source of truth. Schema lives in [`apps/wiki-vite/convex/schema.ts`](../../convex/schema.ts). Every tenant-owned row carries an optional `siteId` (optional only because of the Diana-era backfill window — new writes always set it).
...
Cross-site leak tests in CI run a fixture with two sites and assert that reads/searches/embeddings against site A never surface site B rows ([`apps/wiki-vite/specs/multi-site.md`](../../specs/multi-site.md)).
