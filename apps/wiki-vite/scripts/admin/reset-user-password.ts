 ...
 *
 * Reads NEXT_PUBLIC_CONVEX_URL (or CONVEX_URL) from apps/wiki-vite/.env.local when --email is provided.
 */
...
if (email) {
  ...
  if (!convexUrl) {
    console.error("NEXT_PUBLIC_CONVEX_URL is not set in apps/wiki-vite/.env.local");
    process.exit(1);
  }
