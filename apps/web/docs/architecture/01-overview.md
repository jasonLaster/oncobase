...
|---|---|
| **Next.js app** (`apps/web/src/app`) | Pages, API routes, the chat runtime, the file/PDF proxy. Deployed to Vercel. |
| **Routing middleware** (`apps/web/src/proxy.ts`) | Resolves the active site from the `Host` header, gates passwords, and forwards `x-site-slug` to every downstream handler. |
| **Convex** (`apps/web/convex`) | Source of truth for documents, conversations, users, comments. Provides full-text + vector search. |
