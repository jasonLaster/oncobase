...
Role permissions are include/exclude path-prefix and tag rules stored in Convex. `sources/private/*` can include that subtree, `sources/private/report` includes that exact prefix, and an include tag such as `serova` can protect matching source documents through either normal page tags or the `sensitive-include` frontmatter field. Legacy `serova-sensitive` and `echo-sensitive` tags/rules still match their canonical `serova` and `echo` sensitive include values. Exclude paths and exclude tags narrow a rule, so a role can cover a subtree or tag while carving out a public-summary path or tag. A slug is considered protected only when it matches at least one role permission for the current site. If it is protected, the signed-in user must have an assigned role whose permissions also match the slug and tags. A user who is merely signed in but lacks a matching role receives the same 404 as an anonymous user.
