...
Artifacts shipped:
- `apps/web/src/lib/chat/perf.ts` — small typed sink, no runtime cost in prod beyond a sampled `performance.mark`.
- `apps/web/e2e/chat-perf.spec.ts` — Playwright scenarios.
- `apps/web/scripts/chat-bench.ts` — CLI bench.
...
Artifacts shipped:
- `apps/web/src/app/api/chat/_flusher.ts` (~120 LOC).
- `apps/web/src/app/api/chat/route.ts` slimmed by the inline flush logic.
...
- Updated `apps/web/src/app/api/chat/_flusher.ts` and `convex/conversations.ts`.
...
Artifacts shipped:
- `apps/web/src/app/(main)/chat/_components/chat-client.tsx` (new, holds `useChat`).
...
Artifacts shipped:
- `apps/web/src/app/(main)/chat/_components/messages.tsx` — `<PriorMessages>` + `<StreamingMessage>`.
- `apps/web/src/app/(main)/chat/_components/message-row.tsx` — memoized row.
- `apps/web/src/app/(main)/chat/_components/parts/*.tsx` — one file per part renderer.
...
Artifacts shipped:
- `apps/web/src/components/chat/markdown.tsx` — thin wrapper over `<Streamdown>`.
- Two ported remark plugins under `apps/web/src/lib/markdown/`.
...
Work:
- `npx ai-elements@latest add conversation prompt-input`. Components land under `apps/web/src/components/ai-elements/`.
...
Artifacts shipped:
- `apps/web/src/app/(main)/chat/_components/composer.tsx` (new).
...
Artifacts shipped:
- `apps/web/src/lib/chat/system-prompt-cache.ts`.
