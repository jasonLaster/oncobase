...
`apps/web/src/proxy.ts` resolves the active site from the `Host` header
...
2. `apps/web/.local-hosts.json` fixture
...
`apps/web/convex/lib/site.ts` exports `requireSite(ctx, siteSlug)`:
...
request via `siteDataFromRequest(request)` in `apps/web/src/lib/site-data.ts`.
...
`apps/web/src/lib/blob.ts`'s `sitePut` helper. ESLint bans direct
...
vault. The CLI hits `apps/web/src/app/api/publish/[...step]/route.ts`
...
`apps/web/e2e/multi-site-isolation.spec.ts` exercises six invariants
