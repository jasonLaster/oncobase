...
import { backendServiceToken, createBackendClient } from "./reference-backend-client";
import { api } from "../../convex/_generated/api";
import { SERVICE_ISSUER, SERVICE_AUDIENCE, SERVICE_SUBJECT } from "./reference-backend-client";
...
test("backend transport sends credentials only in headers to its configured origin", async () => {
  ...
  await client.query(api.sites.getBySlug, { slug: "diana" });
  expect(calls).toBe(1);
  delete process.env.WIKI_BACKEND_SIGNING_KEY;
  await expect(client.query(api.sites.getBySlug, { slug: "diana" })).rejects.toThrow("not configured");
  expect(calls).toBe(1);
});

test("browser token grants only site-scoped conversations and binds the current gate", async () => {
  process.env.WIKI_BACKEND_SIGNING_KEY = JSON.stringify(privateJwk);
  const { browserConversationToken } = await import("./reference-backend-client");
  const site = { _id: "fixture-site", slug: "diana", config: { passwordGate: true, passwordHash: "fixture-hash" } };
  const token = await browserConversationToken(site);
  const [header, payload, signature] = token.split(".") as [string,string,string];
  expect(await crypto.subtle.verify("RSASSA-PKCS1-v1_5", pair.publicKey, decode(signature), new TextEncoder().encode(header + "." + payload))).toBe(true);
  const claims = JSON.parse(new TextDecoder().decode(decode(payload)));
  expect(claims).toMatchObject({ role: "wiki-conversations", sub: "wiki-browser:diana", siteSlug: "diana" });
  expect(claims.exp - claims.iat).toBe(60);
  expect(claims.gateVersion).toBe(new Bun.CryptoHasher("sha256").update(JSON.stringify([site._id, true, "fixture-hash"])).digest("hex"));
  const rotated = await browserConversationToken({ ...site, config: { ...site.config, passwordHash: "rotated-hash" } });
  const rotatedClaims = JSON.parse(new TextDecoder().decode(decode(rotated.split(".")[1]!)));
  expect(rotatedClaims.gateVersion).not.toBe(claims.gateVersion);
});
