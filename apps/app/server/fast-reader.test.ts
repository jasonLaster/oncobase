...
test("fast reader makes one fresh policy/content read, checks the gate and invalidates rendered redactions", async () => {
  ...
  const client = { query: async () => { calls++; return { siteSlug: "diana", gate: { enabled: true, passwordHash },
    piiPatterns: [JSON.stringify({ pattern: "PERSON_LITERAL", replacement })],
    page: visible ? { slug: "index", title: "Home", content: "PERSON_LITERAL", contentHash: "same", sensitive: false as const, tags: [] } : null }; } };
