...
```ts
// apps/app/src/app/(main)/chat/_components/messages.tsx
...
t3code: `~/src/os/t3code/apps/apps/app/src/store.ts:128–378`
...
`apps/app/src/app/(main)/chat/_components/`:
...
5. **Add a render-counter dev tool.** A small overlay that shows commits/sec for `<ChatClient>`, `<PriorMessages>`, `<StreamingMessage>` so we can spot regressions. The Phase 0 perf buffer infra (`apps/app/src/lib/chat/perf.ts`) already supports this; add a UI.
