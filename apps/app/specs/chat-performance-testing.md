...
3. **Playwright** — a `chat-perf` project under `apps/app/e2e/`. Each scenario captures a CDP performance trace plus a JSON line written to `apps/app/e2e/.perf/`.
...
Budgets are tracked in `apps/app/e2e/.perf/budget.json`. Playwright fails the run if a metric exceeds `budget * 1.1` (10% headroom).

## Baseline scenarios (Phase 0)

These are the canonical scenarios we run before and after every phase. They live in `apps/app/e2e/chat-perf.spec.ts`.
...
Functional behavior we must preserve at every phase. Each row is a Playwright assertion in `apps/app/e2e/chat-functional.spec.ts`.
...
A snapshot suite under `apps/app/e2e/markdown-gallery.spec.ts` renders fixed strings and screenshot-diffs them. New rows added during Phase 5 to lock in Streamdown parity.
...
Tolerance: per-pixel diff with `maxDiffPixels: 200, threshold: 0.2`. Failures land in `apps/app/e2e/__diff_output__/`.
...
- Chrome DevTools Protocol traces are saved as `apps/app/e2e/.perf/<scenario>-<sha>.trace.json` so we can diff Performance panels across phases.
- The `chat-bench.ts` script accepts `--baseline` to write into `apps/app/e2e/.perf/baseline.json` and `--compare` to diff against it.
