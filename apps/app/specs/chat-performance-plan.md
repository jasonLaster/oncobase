...
Artifacts shipped:
- `apps/app/src/lib/chat/perf.ts` — small typed sink, no runtime cost in prod beyond a sampled `performance.mark`.
- `apps/app/e2e/chat-perf.spec.ts` — Playwright scenarios.
- `apps/app/scripts/chat-bench.ts` — CLI bench.
...
- Extract `createConvexFlusher(conversationId, { intervalMs: 250 })` to `apps/app/src/app/api/chat/_flusher.ts`. It owns:
...
Artifacts shipped:
- `apps/app/src/app/api/chat/_flusher.ts` (~120 LOC).
- `apps/app/src/app/api/chat/route.ts` slimmed by the inline flush logic.
...
- Updated `apps/app/src/app/api/chat/_flusher.ts` and `convex/conversations.ts`.
...
Work:
- Update `apps/app/src/app/api/chat/route.ts` to return `result.toUIMessageStreamResponse({ generateId })`. The existing Convex flusher still runs as a side effect.
...
Artifacts shipped:
- `apps/app/src/app/(main)/chat/_components/chat-client.tsx` (new, holds `useChat`).
...
Artifacts shipped:
- `apps/app/src/app/(main)/chat/_components/messages.tsx` — `<PriorMessages>` + `<StreamingMessage>`.
- `apps/app/src/app/(main)/chat/_components/message-row.tsx` — memoized row.
- `apps/app/src/app/(main)/chat/_components/parts/*.tsx` — one file per part renderer.
...
Artifacts shipped:
- `apps/app/src/components/chat/markdown.tsx` — thin wrapper over `<Streamdown>`.
- Two ported remark plugins under `apps/app/src/lib/markdown/`.
...
Work:
- `npx ai-elements@latest add conversation prompt-input`. Components land under `apps/app/src/components/ai-elements/`.
...
Artifacts shipped:
- `apps/app/src/app/(main)/chat/_components/composer.tsx` (new).
...
Artifacts shipped:
- `apps/app/src/lib/chat/system-prompt-cache.ts`.
