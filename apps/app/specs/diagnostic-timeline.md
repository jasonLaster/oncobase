...
`apps/app/e2e/diagnostic-timeline.spec.ts`. Route, header, sidebar, legacy
redirect, and diagnostics subpage regressions are covered by
`apps/app/e2e/diagnostics-regression.spec.ts`.
...
`apps/app/e2e/diagnostic-timeline.spec.ts` verifies:
...
`apps/app/e2e/diagnostics-regression.spec.ts` verifies:
