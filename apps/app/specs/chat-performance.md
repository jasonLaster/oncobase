...
In scope:
- The chat route at `apps/app/src/app/(main)/chat/`, including `[id]/page.tsx`, `[id]/client.tsx`, and `_components/chat-interface.tsx`.
- The chat API route at `apps/app/src/app/api/chat/route.ts`.
...
Frontend findings (paths under `apps/app/src/app/(main)/chat/`):
...
Backend findings (`apps/app/src/app/api/chat/route.ts`):
...
```ts
// apps/app/src/lib/chat/types.ts (new)
...
`apps/app/src/app/api/chat/route.ts` is reorganized to look like this (sketch — full code in the plan):
