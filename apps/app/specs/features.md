# Historical Next Feature Inventory

Historical inventory of the retired `apps/web` implementation. Product contracts remain useful, but references to Next routing, rendering, and middleware are not current Vite implementation guidance. See [the app guide](../README.md) and [architecture](../docs/architecture/README.md) for current ownership.
...
- Timeline interactions include:
  ...
  - toggleable track pills and `Cmd + scroll` zooming inside category
    magnifier dialogs
  - popup tooltips inside magnifier dialogs
  - sticky left-side color-coded y-axes, vertical axis titles, and aligned
    markers for grouped drill-in charts
