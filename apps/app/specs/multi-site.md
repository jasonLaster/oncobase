...
`apps/app/src/proxy.ts` resolves the active site from the `Host` header
...
2. `apps/app/.local-hosts.json` fixture
...
`apps/app/convex/lib/site.ts` exports `requireSite(ctx, siteSlug)`:
...
request via `siteDataFromRequest(request)` in `apps/app/src/lib/site-data.ts`.
...
`apps/app/src/lib/blob.ts`'s `sitePut` helper. ESLint bans direct
...
vault. The CLI hits `apps/app/src/app/api/publish/[...step]/route.ts`
...
`apps/app/e2e/multi-site-isolation.spec.ts` exercises six invariants
