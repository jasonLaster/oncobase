...
| State | Cue |
| --- | --- |
| JavaScript still loading after 600 ms | A softly breathing D mark above “Launching Diana TNBC...”. The initial HTML starts hidden, preventing an unstyled launch flash on warm visits. |
| Code ready, identity or storage pending | Use the quiet page-loading cue; never show the app-launch label for a data or database wait. |
...
Page and navigation cues wait 180 ms before a 160 ms opacity reveal, avoiding flashes on fast loads. Pulse duration is 2.4 seconds. Reduced-motion preferences disable these animations and icon transitions; the launch indicator's visibility delay remains, without motion. Page cues do not intercept pointer events; the mobile position clears the existing floating action. Status is announced politely without marking the usable file tree busy.
