...
- Let each role grant one or more path-prefix permissions.
...
| `rolePermissions` | `siteId`, `roleId`, `pathPattern` | Prefix-style path patterns granted by the role. |
...
A slug is protected only if it matches at least one `rolePermissions.pathPattern` for the current site. If no role permission matches, the slug remains public. If a role permission matches, the user must be signed in and have an assigned role with a matching permission.
...
- create a role with comma-separated path patterns
- list roles and their path permissions
