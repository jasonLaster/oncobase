...
When the suite runs, paste the JSON dumps from `web/e2e/.perf/` here and update the rows below.
...
Code: `web/src/app/api/chat/_flusher.ts` (new), `web/src/app/api/chat/route.ts` (smoothStream + flusher delegation).
...
Code: `web/src/components/chat/streaming-markdown.tsx`, behind `NEXT_PUBLIC_CHAT_STREAMDOWN=1`.
...
Code: `web/src/app/api/chat/_system-prompt-cache.ts` (60s TTL, single in-flight promise), `messages.messageId` + `by_message_id` index, idempotent `saveMessages`, `createIdGenerator`, `x-request-id` header + log prefix.
