...
All scripts read `web/.env.local` for `NEXT_PUBLIC_CONVEX_URL` and,
...
4. Tell the publisher to run `wiki:init`, set their token in
   ...
   `web/.env.local`), then `wiki:check` and `wiki:publish`.
...
2. Init the publisher CLI:
   ...
   ```
3. Pull `BLOB_READ_WRITE_TOKEN` into `web/.env.local` if asset
