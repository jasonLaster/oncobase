...
In scope:
- The chat route at `web/src/app/(main)/chat/`, including `[id]/page.tsx`, `[id]/client.tsx`, and `_components/chat-interface.tsx`.
- The chat API route at `web/src/app/api/chat/route.ts`.
...
Frontend findings (paths under `web/src/app/(main)/chat/`):
...
Backend findings (`web/src/app/api/chat/route.ts`):
...
`web/src/app/api/chat/route.ts` is reorganized to look like this (sketch — full code in the plan):
