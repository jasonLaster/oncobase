...
// passes a directory pathspec and filters to .md in JS, so the
// parser must correctly handle multiple directory depths.

describe("slugsFromGitOutput", () => {
  it("strips the vault prefix and the .md suffix", () => {
    const raw = [
  ...
  it("leaves paths outside the vault prefix intact", () => {
    ...
    const raw = ["web/scripts/foo.md", "obsidian/index.md"].join("\n");
    const slugs = slugsFromGitOutput(raw, "obsidian");
    expect(slugs.has("index")).toBe(true);
    expect(slugs.has("web/scripts/foo")).toBe(true);
  });
});
