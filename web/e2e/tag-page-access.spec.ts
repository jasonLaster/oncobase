...
const RUN_NONCE = `${Date.now().toString(36)}${crypto
  .randomBytes(2)
  .toString("hex")}`;
const SITE_SLUG = `tag-page-${RUN_NONCE}`;
...
const TAG = `serova-e2e-${RUN_NONCE}`;
...
async function signUp(context: BrowserContext, email: string, url: string) {
  ...
  const sessionCookie = response
    ...
    ?.split(";")[0]
    ?.split("=");
  expect(sessionCookie?.[0]).toBe("wiki_user_session");
  expect(sessionCookie?.[1]).toBeTruthy();
...
test.describe("tag page access", () => {
  ...
  test.beforeAll(async () => {
    ...
    await Promise.all([
      ...
      convex.mutation(api.documents.upsert, {
        ...
        tags: [TAG, "serova", "serova-sensitive"],
  ...
  test.afterAll(async () => {
    ...
    try {
      await Promise.all([
        convex.mutation(api.documents.deleteBySlug, {
          siteSlug: SITE_SLUG,
          slug: PUBLIC_SLUG,
        }),
        convex.mutation(api.documents.deleteBySlug, {
  ...
  test("filters tagged pages for logged out, normal, and admin users", async ({
    baseURL,
    browser,
  }) => {
    ...
    try {
      await signUp(normalContext, NORMAL_EMAIL, url);
      await signUp(adminContext, OWNER_EMAIL, url);

      const [normalAccount, adminAccount] = await Promise.all([
        convex.query(api.users.getByEmailForAuth, {
          siteSlug: SITE_SLUG,
          email: NORMAL_EMAIL,
        }),
        convex.query(api.users.getByEmailForAuth, {
      ...
      await anonymousPage.goto(`/tags/${TAG}?viewer=anonymous`, {
      ...
      await anonymousPage.locator("summary", { hasText: "sources" }).click();
      await expect(anonymousPage.getByRole("link", { name: PUBLIC_TITLE })).toBeHidden();
      await anonymousPage.locator("summary", { hasText: "sources" }).click();
      await expect(anonymousPage.getByRole("link", { name: PUBLIC_TITLE })).toBeVisible();
      ...
      await expect
        .poll(async () => {
          const session = await normalContext.request.get("/api/auth/session");
          const body = (await session.json()) as { user?: { email?: string } | null };
          return body.user?.email ?? null;
        })
        .toBe(NORMAL_EMAIL);
      await normalPage.goto(`/${ALLOWED_SLUG}`, { waitUntil: "domcontentloaded" });
      await expect(normalPage.getByRole("heading", { name: ALLOWED_TITLE })).toBeVisible();

      await normalPage.goto(`/tags/${TAG}?viewer=normal`, {
        waitUntil: "domcontentloaded",
      });
      ...
      await expect(normalPage.locator("summary", { hasText: "meeting-notes" })).toBeVisible();
      await expect(normalPage.getByRole("link", { name: DENIED_TITLE })).toHaveCount(0);

      const adminPage = await adminContext.newPage();
      await adminPage.goto(`/tags/${TAG}?viewer=admin`, {
        waitUntil: "domcontentloaded",
      });
      await expect(adminPage.getByText("3 pages")).toBeVisible();
      await expect(adminPage.getByRole("link", { name: PUBLIC_TITLE })).toBeVisible();
      await expect(adminPage.getByRole("link", { name: ALLOWED_TITLE })).toBeVisible();
      await expect(adminPage.getByRole("link", { name: DENIED_TITLE })).toBeVisible();
    } finally {
      void anonymousContext.close().catch(() => {});
      void normalContext.close().catch(() => {});
      void adminContext.close().catch(() => {});
    }
  });
});
