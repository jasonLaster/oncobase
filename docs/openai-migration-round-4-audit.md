...
| P2 | `/table-examples`, documented parity surface | Vite rendered only markdown fixtures, while legacy also exposed the direct React `SmartTable` scenarios and resize-audit index. Vite kept the declarative and landscape-overflow contracts as explicit skips. | The declarative showcase now lives in the shared package and both runtimes pass it their rail-aware adapter. Vite mounts the legacy-equivalent title, description, comments wrapper, scenario inventory, and markdown section; all 10 Vite example stories and all 11 legacy outcomes pass with no table-example skips (`8745f5d4`). |
