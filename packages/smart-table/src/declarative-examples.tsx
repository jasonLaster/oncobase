...
import type { SmartTableLayoutAdapter } from "./layout-adapter";
import {
  SmartTable,
  SmartTableBody,
  SmartTableCell,
...
const resizeAuditIds = new Set(
  resizeAuditExampleTables.map((example) => example.id),
);
...
function ScenarioTable({
  ...
  return (
    <SmartTable layoutAdapter={layoutAdapter} persistenceKey={persistenceKey}>
      <SmartTableHeader>
        <SmartTableRow>
      ...
      <SmartTableBody>
        {example.rows.map((row) => {
          const rowKey = `${example.id}:${row.join("\u001f")}`;
          return (
            <SmartTableRow key={rowKey}>
              {row.map((cell, cellIndex) => (
                <SmartTableCell
                  key={`${rowKey}:${example.headers[cellIndex] ?? cell}`}
                >
                  {cell}
                </SmartTableCell>
              ))}
...
export function DeclarativeSmartTableShowcase({
  layoutAdapter,
}: {
  layoutAdapter: SmartTableLayoutAdapter;
}) {
    ...
    <section data-smart-table-showcase className="mb-12 space-y-10">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-[var(--sidebar-border)] bg-[var(--sidebar-bg)]/45 p-5">
          ...
          <p className="mt-3 text-3xl font-semibold">
            {resizeAuditExampleTables.length}
          </p>
          <p className="mt-2 text-sm text-[var(--text-muted)]">
      ...
      <section
        data-smart-table-performance-summary
      ...
      >
        <div className="max-w-4xl">
          ...
          <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">
            These scenarios are the ones to drag when we care about 60fps. They
            cover long wrapped prose, compact numeric columns, and a table that
            is already overflowing before any manual resize happens.
          </p>
        </div>
